...
def generate_gaff(
    ...
    return_names:
        Returns the Residue name, MOL2 file name and FRCMOD file name.
    """

    if output_name is None:
        output_name = mol2_file.split(".mol2")[0]
    ...
    if need_gaff_atom_types:
        ...
        if len(structure.residues) > 1:
            structure[":1"].save(os.path.join(directory_path, "tmp.mol2"))
            if os.path.exists(os.path.join(directory_path, "tmp.mol2")):
                os.rename(
                    os.path.join(directory_path, "tmp.mol2"),
                    os.path.join(directory_path, f"{output_name}.{gaff_version}.mol2"),
                )
    ...
    if return_names:
        return residue_name, f"{output_name}.{gaff_version}.mol2"
